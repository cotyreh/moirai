...
load_data <- function() {
  gexvol <- read.csv("data/historical/gexvol.csv")
  initialprint <- read.csv("data/historical/initialprint.csv")
  ...
  file_name <-
    paste0("data/intraday/intradayGEX", current_date, ".csv")
...
generate_snapshot <-
  function(time_step,
    ...
    title <-
      paste(current_date, "|", recent_price, "as of", time_labels[time_step])
    ...
    p <- ggplot() +
      ...
      geom_hline(yintercept = previous_day_close, linetype = "dotdash") +
      scale_x_discrete(
        name = "",
    ...
    pp <- ggbackground(p, "www/paper.JPG")
    
    # Save each plot as a separate PNG
    file_name <-
      paste0(snapshot_folder,
             "/snapshot_",
             sprintf("%03d", time_step),
             ".png")
    ggsave(
      file_name,
      plot = pp,
...
create_gif <- function(snapshot_folder) {
  image_files <-
    list.files(path = snapshot_folder,
               pattern = "snapshot_.*\\.png",
               full.names = TRUE)
  if (length(image_files) > 0) {
    gif <- image_read(image_files)
    animation <- image_animate(gif, fps = 5)
    image_write(animation, "forecast_animation.gif")
  } else {
    warning("No image files found. Please check the file paths.")
  }
}
...
run_simulation <- function() {
  ...
  # Generate and save snapshots for each time step
  for (i in 1:length(data$realized_prices)) {
    generate_snapshot(
      i,
      snapshot_folder,
  ...
  # Create GIF from the saved snapshots
  create_gif(snapshot_folder)
}
